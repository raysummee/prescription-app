...
class AddMedicineBloc extends Bloc<AddMedicineEvent, AddMedicineState> {
  ...
  AddMedicineBloc(this.repository) : super(const AddMedicineUpdated()) {
    ...
    on<MedicineNameChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(medicineName: event.medicineName));
    });
    ...
    on<DoseChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(dose: double.tryParse(event.dose) ?? 0));
    });
    ...
    on<DoseTypeChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(doseType: event.doseType));
    });
    ...
    on<StomachChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(stomach: event.stomach));
    });
    ...
    on<TimeChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(time: event.time));
    });
    ...
    on<CommentsChange>((event, emit) {
      ...
      final stateUpdated = state as AddMedicineUpdated;
      emit(stateUpdated.copyWith(comments: event.comments));
    });
    ...
    on<AddMedicine>((event, emit) async {
      ...
      if (stateUpdated.medicineCategoryIndex == null ||
          stateUpdated.medicineCategoryIndex! < 0) {
        emit(const AddMedicineError(
            message: "Invalid values or missing values"));
        emit(stateUpdated);
        return;
      }
      if (stateUpdated.medicineCategoryIndex == null ||
